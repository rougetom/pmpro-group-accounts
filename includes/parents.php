...
function pmprogroupacct_pmpro_after_checkout_parent( $user_id ) {
	...
	$active_by_order = array();
	foreach ( $group->get_active_members() as $active_member ) {
		if ( ! empty( $active_member->child_order ) ) {
			$active_by_order[ (int) $active_member->child_order ] = $active_member;
		}
	}
	...
	for ( $i = 0; $i < $child_count; $i++ ) {
		...
		if ( isset( $active_by_order[ $profile['child_order'] ] ) ) {
			$active_by_order[ $profile['child_order'] ]->update_profile( $profile );
			continue;
		}
...
function pmprogroupacct_ajax_render_child_fields() {
	...
	if ( empty( $level_id ) || ! pmprogroupacct_level_is_multi_child_parent( $level_id ) ) {
